...
def datingClassTestf():
    # x = 0.10  # hold out 10%
    datingDataMat, datingLabels = file2matrix(
        'fruit.csv')  # load data setfrom file
    normMat, ranges, minVals = autoNorm(datingDataMat)
    m = normMat.shape[0]

    datingDataMatT, datingLabelsT = file2matrix(
        'testFruit.csv')  # load data setfrom file
    normMatT, rangesT, minValsT = autoNorm(datingDataMatT)
    mT = normMatT.shape[0]
    ...
    errorCount = 0.0
    for i in range(numTestVecs):
        classifierResult = classify0(normMatT[i, :], normMat, datingLabels, 3)
        print "the classifier came back with: %s, the real answer is: %s" % (classifierResult, datingLabelsT[i])
        if (classifierResult != datingLabelsT[i]):
            errorCount += 1.0
    print "the total error rate is: %f" % (errorCount / float(numTestVecs))
    print errorCount
...
def classifyFruit():
    ...
    redness = float(
        raw_input("redness?"))
    yellowness = float(raw_input("yellowness?"))
    mass = float(raw_input("mass?"))
    volume = float(raw_input("volume?"))
    datingDataMat, datingLabels = file2matrix('fruit.csv')
    normMat, ranges, minVals = autoNorm(datingDataMat)
    inArr = array([redness, yellowness, mass, volume])
    classifierResult = classify0(
        (inArr - minVals) / ranges, normMat, datingLabels, 3)
    print "You will probably like this fruit: ", resultList[classifierResult]
    # print "\t[Info] You will probably like this person:
    # {0}!".format(resultList = {'didntLike': 'not at all', 'smallDoses': 'in
    # small doses', 'largeDoses': 'in large doses'})
